...
public class playerScript_ex05 : MonoBehaviour {
	...
	void OnTriggerEnter2D (Collider2D col) {
		if (col.CompareTag ("Bullet") && col.gameObject.layer == 8 && name == "Claire") {
			Application.LoadLevel (5);
		} else if (col.CompareTag ("Trap"))
			Application.LoadLevel (5);
		else if (col.CompareTag ("Bullet") && col.gameObject.layer == 9 && name == "Thomas") {
			Application.LoadLevel(5);
		}
		else if (col.CompareTag ("Bullet") && col.gameObject.layer == 10 && name == "John") {
			Application.LoadLevel(5);
		}
	}
}
